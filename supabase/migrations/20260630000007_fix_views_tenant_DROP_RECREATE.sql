-- ==========================================
-- FIX: Isolamento de Tenant em VIEWS (DROP + RECREATE)
-- ==========================================
-- PROBLEMA: CREATE OR REPLACE não funciona quando muda estrutura
-- SOLUÇÃO: DROP todas as views e recriar com filtro tenant_id
-- ==========================================

-- ==========================================
-- PASSO 1: DROP TODAS AS VIEWS (ordem reversa de dependência)
-- ==========================================
...
GROUP BY f.id, f.nome_fantasia, f.cnpj, f.tenant_id, f.status
...
-- 9️⃣ VW_DASHBOARD_KPIS (se existir, recriar)
...
-- 🔟 VW_FILIAIS_COMPLETO (se existir)
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════╗';
  RAISE NOTICE '║   ✅ VIEWS RECRIADAS COM ISOLAMENTO       ║';
  RAISE NOTICE '╚════════════════════════════════════════════╝';
  RAISE NOTICE '';
  RAISE NOTICE '✅ 10 views críticas recriadas com filtro tenant_id';
  RAISE NOTICE '✅ Todas usam: WHERE tenant_id IN (SELECT tenant_id FROM profiles WHERE id = auth.uid())';
  RAISE NOTICE '✅ security_invoker = true aplicado';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  TESTE AGORA:';
  RAISE NOTICE '   1. Faça LOGOUT e LOGIN';
  RAISE NOTICE '   2. Verifique se produtos de outro tenant sumiram';
  RAISE NOTICE '';
END $$;
