-- ==========================================
-- FIX VIEWS: Isolamento de Tenant (SEM REMOVER CAMPOS)
-- ==========================================
-- Mantém TODAS as colunas das views originais
-- Apenas adiciona filtro de tenant_id
-- ==========================================
...
-- 1️⃣ VW_PRODUTOS_MAIS_REQUISITADOS (mantém estrutura original + filtro tenant)
...
-- 2️⃣ VW_PRODUTOS_POR_CLASSIFICACAO (da migration original de classificação + filtro tenant)
...
-- 3️⃣ VW_PRODUTOS_CRITICOS (usa apenas colunas que EXISTEM)
...
-- 7️⃣ VW_APROVACOES_PENDENTES (usa estrutura mais recente: tipo_documento + documento_id)
...
-- 8️⃣ VW_ITENS_REQUISICAO_COMPLETO (da migration original + filtro tenant)
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '╔════════════════════════════════════════════╗';
  RAISE NOTICE '║   ✅ VIEWS COM ISOLAMENTO DE TENANT       ║';
  RAISE NOTICE '╚════════════════════════════════════════════╝';
  RAISE NOTICE '';
  RAISE NOTICE '✅ 10 views recriadas mantendo TODOS os campos';
  RAISE NOTICE '✅ Filtro aplicado: WHERE tenant_id IN (SELECT tenant_id FROM profiles WHERE id = auth.uid())';
  RAISE NOTICE '✅ security_invoker = true em todas';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️  FAÇA LOGOUT E LOGIN para testar!';
  RAISE NOTICE '';
END $$;
